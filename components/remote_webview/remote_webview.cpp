...
void RemoteWebView::ws_task_tramp_(void *arg) {
  ...
  for (;;) {
    ...
    if (!esp_websocket_client_is_connected(client)) {
      std::string uri = self->build_ws_uri_();
      ESP_LOGI(TAG, "[ws] reconnect with uri: %s", uri.c_str());
      esp_websocket_client_set_uri(client, uri.c_str());
      esp_websocket_client_start(client);
      continue;
    }
...
void RemoteWebView::set_server(const std::string &s) {
  ...
  std::string new_host = s.substr(0, pos);
  int new_port = atoi(s.c_str() + pos + 1);
  ...
  ESP_LOGI(TAG, "server updated to %s:%d", server_host_.c_str(), server_port_);

  if (ws_client_ != nullptr) {
    std::string uri = build_ws_uri_();
    ESP_LOGI(TAG, "updating websocket uri to %s", uri.c_str());

    esp_websocket_client_stop(ws_client_);
    esp_err_t err = esp_websocket_client_set_uri(ws_client_, uri.c_str());
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "esp_websocket_client_set_uri failed: %d", (int) err);
      return;
    }
